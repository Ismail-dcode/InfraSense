...
import { Sparkles, CheckCircle2, ShieldCheck, DollarSign, Cpu, Zap, HardDrive, Wifi, Copy, Check, Download, Layers, Code2, Shield, Bookmark, HelpCircle, Network, Activity, Lock, ArrowRight, Scale, ListChecks } from 'lucide-react';
...
export default function RecommendationResult({ result, userInput }) {
  ...
  const workloadName = WORKLOAD_PROFILES.find((profile) => profile.id === userInput.workload)?.name || userInput.workload;
  const requestedFeatures = [
    userQuestions.needMonitoring && '24/7 monitoring',
    userQuestions.needLoadBalancer && 'load balancing',
    userQuestions.needAutoScaling && 'auto scaling',
    userQuestions.needMultiAZ && 'high availability',
    userQuestions.needEncryption && 'encryption'
  ].filter(Boolean);
    ...
    <div className="max-w-5xl mx-auto space-y-6 sm:space-y-8 animate-fadeIn">
      ...
      {activeOutputTab === 'specs' && (
        <div className="space-y-6 animate-fadeIn">
          ...
          {/* Structured context makes the recommendation explainable and reproducible. */}
          <div className="bg-white border border-slate-200/90 rounded-3xl p-6 sm:p-8 space-y-4 shadow-sm">
            <div className="border-b border-slate-100 pb-4">
              ...
              <h3 className="text-xl font-extrabold text-slate-900 flex items-center gap-2 mt-0.5">
                ...
                Inputs considered for this recommendation
              </h3>
              <p className="text-xs text-slate-500 mt-1">
                These requirements were evaluated together with the matching rules below.
              </p>
            </div>

            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 text-sm">
              <div className="bg-slate-50 rounded-xl border border-slate-200 p-3">
                <span className="block text-[10px] font-mono font-bold uppercase text-slate-500">Workload</span>
                <span className="font-bold text-slate-900">{workloadName}</span>
              </div>
              <div className="bg-slate-50 rounded-xl border border-slate-200 p-3">
                <span className="block text-[10px] font-mono font-bold uppercase text-slate-500">Requested capacity</span>
                <span className="font-bold text-slate-900">{userInput.vcpu} vCPU · {userInput.ram} GB RAM</span>
              </div>
              <div className="bg-slate-50 rounded-xl border border-slate-200 p-3">
                <span className="block text-[10px] font-mono font-bold uppercase text-slate-500">Deployment</span>
                <span className="font-bold text-slate-900">{userInput.instanceCount} instance{userInput.instanceCount === 1 ? '' : 's'} · {userInput.provider.toUpperCase()}</span>
              </div>
              <div className="bg-slate-50 rounded-xl border border-slate-200 p-3">
                <span className="block text-[10px] font-mono font-bold uppercase text-slate-500">Traffic</span>
                <span className="font-bold text-slate-900">{userInput.trafficPattern || 'Not specified'}</span>
              </div>
              <div className="bg-slate-50 rounded-xl border border-slate-200 p-3">
                <span className="block text-[10px] font-mono font-bold uppercase text-slate-500">Priority</span>
                <span className="font-bold text-slate-900">{userInput.budgetPriority}</span>
              </div>
              <div className="bg-slate-50 rounded-xl border border-slate-200 p-3 sm:col-span-2 lg:col-span-1">
                <span className="block text-[10px] font-mono font-bold uppercase text-slate-500">Requested services</span>
                <span className="font-bold text-slate-900">{requestedFeatures.length ? requestedFeatures.join(', ') : 'Base compute only'}</span>
              </div>
            </div>
          </div>
